...
> 세션 핸드오프 (2026-08-04):
> - [Bug Fix] 15분 간격 스케줄러 실행 시 `[D-1] 브라질 Copom` 캘린더 알림이 15분마다 중복 반복 발송되던 현상 해결.
> - `SectorInsight` DB에 `brazil_cal_alert_{event}_{tag}_{date}` 일일 발송 기록을 추적하여, D-1/D-DAY 캘린더 알림은 하루에 딱 1회만 발송되도록 중복 차단 구현.
