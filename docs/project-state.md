...
✅ Current: Sprint 5 — 글로벌 매크로 스트레스 테스트, 환율/절세 시뮬레이터, 우주 및 바이오 ETF 구성종목 변동 그래프/개별 주식 팝업 연동, 특정 ETF 선택 시 테이블/차트 구성종목 10종 필터링 고도화, My 탭 내 보유 자산 정보 기반 AI Assistant 서비스 연동/바로가기 위젯 추가 및 우주섹터 미국 신규 ETF 5종 추가/한-미 마켓 토글 전환 테이블 고도화 완료
...
|----|-------|--------|-------|
| S5-7 | 우주섹터 미국 신규 ETF 5종 연동 및 한/미 마켓 토글 테이블 고도화 | ✅ done | 비교 차트에 미국 ETF 5종(UFO/MARS/NASA/ORBX/WARP) 추가, 범례 레이아웃 분리(국내 윗줄/미국 아랫줄), 테이블 상단 한/미 토글 버튼 및 비중 동적 렌더링 구현 완료 |
