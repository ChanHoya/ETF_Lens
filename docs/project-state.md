...
✅ Current: Sprint 6 — 브라질 채권 금리 다중 시각화(Option A + B) 및 10Y/1Y/6M/3M 조회 범위 옵션 필터 추가 완료 (2026-07-13)
➡️ Next: S6-4 ETF 배당(분배금) 정보 수집 백엔드 스크래퍼 및 API

> 세션 핸드오프 (2026-07-13):
> - 브라질 채권 금리 사이클 차트에서 FRED 역사적 월간 데이터(주황색)와 Investing.com 실제 최근 22일 일별 데이터(초록색)를 병합하고 선형 보간하여 연결 지점이 튀는 문제를 보정하였습니다.
> - 차트 하단에 역사적 국채금리(FRED)와 실제 국채금리(Investing.com)의 두 가지 산정기준 및 연결 처리 방식 가이드를 추가했습니다.
> - 금리사이클 조회 기준 범위를 기본 10년(10Y)에서 1Y, 6M, 3M로 손쉽게 전환할 수 있는 옵션 버튼을 연동하여 0ms 반응 속도로 필터링되도록 구성했습니다.
