...
✅ Current: Sprint 6 — S6-8 (전력/에너지 섹터 주요 종목 현황 및 구성종목 비중 비교) 완료 (2026-06-02)
...
> - 핫픽스 4: 자산 추이 과거 데이터를 최초 1회 연산 후 DB에 Bulk 적재하여 캐싱하고, 다음 호출 시 DB에서 **1ms** 만에 즉시 반환하도록 최적화(매번 느린 KIS 조회 대기 해결) 및 선택 기간의 첫 날(시작일)의 자산금액과 실제 누적수익률(원본 수치)이 Y축 가로 상의 동일한 물리적 높이(기준선)에서 일치하여 교차 렌더링되도록 basePrincipal 기준의 1:1 비례 도메인 동기화 공식 적용 완료
> - 핫픽스 5: 우주섹터 주요 ETF 구성종목 테이블 헤더의 "익일예상" 문구를 "예상가격"으로 명확히 개선하고 산출 기준을 현재가 대신 '전일 한국 거래 종가(prev_close) * (미국 종가 기준 예상 등락률)'로 바로잡아 오차 해결. 아울러 실제가격과 괴리율을 한국 시각 장 시작 전에는 "-"로 표기하고, 개장 이후에는 실시간 실제가격과 함께 계산된 예상가격 기반 괴리율을 실시간 노출하도록 보완 완료
...
| S6-8 | 전력/에너지 섹터 주요 종목 현황 및 구성종목 비중 비교 | ✅ stable | 국내/미국 전력/에너지 주요 ETF 8종의 가격 비교 차트 및 구성종목 비중 테이블 연동 완료 |
## Module Registry
