...
|----|-------|--------|-------|
| S5-14 | TFF 종목별 수익률 엑셀 원본 테이블 구현 및 예수금 핫픽스 | ✅ done | 종목별 수익률 탭 내 엑셀 레이아웃 그대로 반영된 요약표 추가, 현금(예수금) 파싱 알고리즘 최적화, 종목명 클릭 시 상세 팝업창 연동 및 공백/변형 종목명 매핑 핫픽스 완료, 미보유 자산 N/A 처리 및 가상 벤치마크 지수 소수점 스케일링 오류 핫픽스 완수, YTM 탭 상단에 엑셀 원본 포맷의 종합 현황판 및 현금/평잔 종합현황 대시보드 추가 완료, 월별분석 탭 상단 셀렉터 헤더 블록의 위치 조정 및 분류 필터 버튼의 종목 카드 리스트 상단 분리 배치 완료 |
