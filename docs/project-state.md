...
✅ Current: PC 마우스 클릭 드래그, 휠 스크롤 및 좌/우 화살표 버튼(< >) 스크롤 제어 구현 완료 (2026-08-06)
➡️ Next: S6-4 ETF 배당(분배금) 정보 수집 백엔드 스크래퍼 및 API

> 세션 핸드오프 (2026-08-06):
> - 메인 메뉴바 PC 환경 조작성 향상: PC에서 마우스 좌클릭 후 좌우 드래그(`cursor-grab`/`cursor-grabbing`), 마우스 휠 스크롤, 그리고 양 옆의 **좌/우 스크롤 화살표 버튼(`<` `>`)**을 통해 클릭 한번으로 간편하게 전체 메뉴를 이동·탐색할 수 있도록 개편.
> - 모바일 터치 드래그 + PC 마우스 제어 통합 구현 완료.
