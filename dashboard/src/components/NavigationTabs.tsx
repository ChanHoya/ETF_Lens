        ...
        <nav className="flex flex-col sm:flex-row space-y-3 sm:space-y-0 sm:space-x-3 w-full sm:w-auto mt-4 sm:mt-0">
            {navItems.map(tab => (
                <button
                    ...
                    onClick={() => {
                        setActiveTab(tab.id);
                        if (isEtfCheckModalOpen) setIsEtfCheckModalOpen(false);
                    }}
                    className={`px-6 py-3 rounded-full text-sm font-bold transition-all duration-300 w-full sm:w-auto relative overflow-hidden group border ${activeTab === tab.id
            ...
            {data && (
                <button
                    onClick={() => {
                        setIsEtfCheckModalOpen(!isEtfCheckModalOpen);
