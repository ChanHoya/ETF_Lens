...
- **설명 가이드 팝업 모달 탑재**: 효율적 전선 차트 우측 상단에 파란색 정보(Info) 아이콘을 추가하고, 클릭 시 현대 포트폴리오 이론(MPT), 최대 샤프(Max Sharpe) / 최소 변동성(Min Variance)의 경제학적 개념, 그리고 차트 해석 방법을 일목요연하게 정리한 가이드 모달을 제공하도록 구성했습니다.
- **최적 비중 비교 바 차트 Y축 글자 잘림 해결**: YAxis의 `width`를 `90`에서 `150`으로 넉넉하게 확장하고, `weightBarData`에서 종목명을 자르는 기준 길이를 기존 10자에서 **16자**로 확대 적용하여 한글 종목명이 모두 온전히 노출되도록 개선했습니다.
- **X축 도메인 최적화**: X축의 고정 범위 `[0, 100]`을 동적 최댓값 기준인 **`[0, 'dataMax']`**로 수정하여 개별 비중이 10~30%인 포트폴리오의 경우 가로 바가 촘촘하게 겹치지 않고 최대 공간을 활용해 시원하게 뻗어나가도록 개선했습니다.
